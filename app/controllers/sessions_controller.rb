  ...
  def create
    ...
    if author&.authenticate(params[:password])
      if author.email_confirmed
        ...
        flash.now[:alert] = 'Пожалуйста, активируйте свою учетную запись, следуя инструкции в электронном письме с подтверждением, которое вы получили, чтобы продолжить'
        render 'new'
      end
    else
